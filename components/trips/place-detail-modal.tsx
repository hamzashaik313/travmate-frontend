...
export function PlaceDetailModal({
  ...
  // ✅ Auto fullscreen when photo viewer opens
  useEffect(() => {
    const elem = viewerRef.current;
    if (photoIndex !== null && elem) {
      if (elem.requestFullscreen) elem.requestFullscreen().catch(() => {});
    } else if (photoIndex === null && document.fullscreenElement) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" && details?.photos)
        setPhotoIndex((prev) =>
          prev !== null ? (prev + 1) % details.photos!.length : prev
        );
      if (e.key === "ArrowLeft" && details?.photos)
        setPhotoIndex((prev) =>
          prev !== null
            ? (prev - 1 + details.photos!.length) % details.photos!.length
            : prev
        );
    };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      {photoIndex !== null && details?.photos && (
        <div
          ref={viewerRef}
          className="fixed inset-0 z-[999999] bg-black flex items-center justify-center overflow-hidden transition-all duration-500"
        >
          {/* Close button */}
          <button
            onClick={() => setPhotoIndex(null)}
            className="absolute top-6 right-8 text-white hover:scale-110 transition"
          >
            ✕
          </button>
          ...
          <img
            ...
            alt="place"
            className="max-w-[100vw] max-h-[100vh] object-contain transition-transform duration-500"
          />
